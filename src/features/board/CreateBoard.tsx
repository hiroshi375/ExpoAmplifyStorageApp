...
function CreateBoard() {
  ...
  // AIプロンプト読み込み関数
  const loadAiSystemPrompt = async () => {
    const savedInstruction = await AsyncStorage.getItem(AI_PROMPT_STORAGE_KEY);
      ...
      <Card>
        <Card.Content>
              ...
              <List.Item
                ...
                left={(props) => (
                  <List.Icon
                    {...props}
                    icon={
                      selectedPersonId === p.id ? "check-circle" : "account"
                    }
                  />
                )}
                style={{
          ...
          <Button
            ...
            style={{ marginTop: 20 }}
            disabled={
              !fmsg || fmsg.trim() === "" || !selectedPersonId || !imagePath // 画像アップロードが完了していない場合は投稿できないようにする
            }
          >
            投稿
          </Button>

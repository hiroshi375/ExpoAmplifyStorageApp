...
// 共通エラーレスポンス
// -----------------------------
const returnError = (statusCode: number, errorCode: string, error: string) => {
  return {
    statusCode,
    body: JSON.stringify({
      ok: false,
...
export const handler = async (event: any) => {
  try {
    ...
    try {
      ...
      if (!imageKey) {
        return returnError(400, "INVALID_REQUEST", "imageKey がありません");
      }
    } catch (e) {
      return returnError(400, "INVALID_JSON", "リクエストJSONが不正です");
    }
    ...
    try {
      image = await s3.send(
        new GetObjectCommand({
          ...
          Bucket:
            "amplify-expoamplifystorag-expostoragebucket60c87bc-xnpfpxaqzafb", // ←環境変数がない場合のフォールバック
          Key: imageKey,
        }),
      );
    } catch (e) {
      console.error("S3 GET ERROR:", e);
      return returnError(500, "S3_ERROR", "画像の取得に失敗しました");
    }
...
`,
    ...
    try {
      ...
      return returnError(500, "BEDROCK_ERROR", "Bedrock呼び出しに失敗しました");
    }
    // -----------------------------
    // Response Parse
    // -----------------------------
    ...
    return returnError(500, "UNKNOWN_ERROR", "不明なエラーが発生しました");
  }
};
